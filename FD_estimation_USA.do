...
clear all
set more off
cd "D:\Copy\GVC\National Accounts\US"

*** From ICIO Table, 2000, 2005, 2008-2011
* See D:\Copy\GVC\National Accounts\US\USATotalFinalDemand.do
...
save "D:\Copy\GVC\ICIO\Stata\NAICS-ICIO_concordance", replace

* Concordance between ICIO & our Classification
import excel using D:\Copy\GVC\ICIO\Excel\ICIO_ciid_classification.xlsx, sheet(iid3d) firstrow clear
rename iid_icio ICIO
rename (ename kname post) (iid_eng iid_kr iid_3d)
save ICIO-iid3d_concordance, replace

joinby ICIO using D:\Copy\GVC\ICIO\Stata\NAICS-ICIO_concordance
...
foreach i of numlist 2005(1)2014 {
	...
	* Collapse Industry Unit according to ICIO
	joinby Name using "D:\Copy\GVC\ICIO\Stata\NAICS-ICIO_concordance"
	order NAICS Name ICIO
	collapse (sum) DFD`i', by(iid_3d)
	export excel using "D:\Copy\GVC\ICIO\excel\FD_estimation_USA.xlsx", sheet(DFD`i') firstrow(var) sheetrep
...
import excel using GrossOutput_1997-2014.xlsx, sheet(GO) cellrange(B7:T107) clear
rename (B Q) (Name GO)
...
joinby Name using D:\Copy\GVC\ICIO\Stata\NAICS-ICIO_concordance
bysort iid_3d: egen wt = pc(GO)
order Name iid_3d wt GO
forval yr=2006/2014  {
	loc lag = `yr'-1
	gen PI`yr' = P`yr'/P`lag'
	gen wPI`yr' = PI`yr'*wt/100
	}
collapse (sum) wPI*, by(iid_3d)
export excel using "D:\Copy\GVC\ICIO\excel\FD_estimation_USA.xlsx", sheet(Price_Index) firstrow(var) sheetrep
