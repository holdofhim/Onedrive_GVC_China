...
# Note: This code calculates the real FD growth effect of a source country on production, value-added, and exports in all countries without the assumption 3 in Bems et al. (2010), the Leontief type of consumer preference. Hence, we don't need  FDD here, but instead we use the (actual) real FD growth rate of the source country for all SN products (SNx1 vector of FD in 'cty.src').

# Caveat: Since we allow that a country's FDs for product s vary across countries, the sectoral growth rate of a source country by itself can't provide us estimates of the implied real growth rate of all countries. For estimation, we need the real FD growth of a source country for all SN products, for which we need sectoral price data for the source country.
...
for (year in period) {
      ...
      Y.alloc <- LeonInv[yr,,]                  # Y.alloc = Output Allocation Matrix
